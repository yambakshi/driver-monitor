...
class ResultRenderer(object):
    def __init__(self, display_fps=False, display_confidence=False, number_of_predictions=1, labels=None,
                 output_height=720):
        ...
        self.vga_resolution = (640, 480)
        self.recorder = cv2.VideoWriter('output.avi', cv2.VideoWriter_fourcc(*'MJPG'), 24, self.vga_resolution, 0)
    ...
    def render_frame(self, frame, logits, timers, frame_ind):
        ...
        # Fill text area
        fill_area(frame, (0, 70), (new_w, 0), alpha=0.6, color=(0, 0, 0))
        ...
        cv2.imshow("Action Recognition", gray_frame)

        # Write frame to output file
        resized_frame = cv2.resize(gray_frame, self.vga_resolution, interpolation=cv2.INTER_AREA)
        self.recorder.write(resized_frame)
